<section class="bg-gold flex flex-col gap-4 p-4 text-l" data-aos="fade-right">
    ...
    <div class="grid mobile:grid-cols-1 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 h-auto gap-8">
        @foreach($best_sellers as $montre)
            <x-cards.montre :$montre />
        @endforeach
        <a href="{{ route("montres.index") }}" class="bg-white sm:h-auto flex flex-col items-center justify-center sm:col-auto gap-1 p-2 text-xl text-center break-words font-semibold hover:underline rounded" data-aos="fade-up">
